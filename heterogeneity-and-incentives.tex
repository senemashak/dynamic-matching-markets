...
four policies (Greedy, Patient, Type-Aware Greedy, Type-Aware
...
public monopoly when markets are thick enough.  These findings build on the kidney-exchange literature defining easy and hard-to-match types. 
\end{abstract}
...
This paper identifies a mechanism for why: when easy-to-match agents face outside options that cause
...
single pool with blended E departure rate.  Future directions include modeling
the joint Markov chain on $(H_t^A,E_t^A,H_t^B,E_t^B)$ and deriving the coupled
...
by occupancy heatmaps that show tight mass near $(h^*,e^*)$.
...
For TA-Patient, the best-performing algorithm, higher $\rho$ compresses
